...
IppRawAttribute::IppRawAttribute(const void *data, size_t size)
	: IppAttribute(IPP_UNKNOWN), bytes((const char *)data, size)
{
}

int IppRawAttribute::length() const
{
	return bytes.length();
}

ostream &IppRawAttribute::output(ostream &os) const
{
	os.write(bytes.data(), bytes.length());
	return os;
}

ostream &IppRawAttribute::print(ostream &os) const
{
	os << "Raw: " << bytes.length() << " bytes\n";
	return os;
}
...
uint32 IppContent::getRequestId() const
{
	return request_id;
}

void IppContent::setRequestId(uint32 i)
{
	request_id = i;
}
...
ostream &IppContent::output(ostream &os) const
{
	...
	uint32 ns_request_id = htonl(request_id);				// request-id
...
void IppContent::setRaw(const void *data, size_t size)
{
	attrs.push_back(new IppRawAttribute(data, size));
}
