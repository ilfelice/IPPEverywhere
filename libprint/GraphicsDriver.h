...
protected:
	...
	// Whether to shrink pages whose content exceeds the printable area
	// so that it fits (only applied when printing one page per sheet).
	// Defaults to the "Scale to fit page" page setup setting.
	virtual	bool	ScaleToFit() const;
			int32	GetPageCount() const { return fPageCount; }
...
private:
			...
			bool	_MeasureContent(PageDataList* pages, BRect& content);
