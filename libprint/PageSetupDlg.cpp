...
#define B_TRANSLATION_CONTEXT "PageSetupDlg"
...
PageSetupView::AttachedToWindow()
{
	...
	if (!marked)
		item->SetMarked(true);
	BMenuField* paperSize = new BMenuField("paperSize", B_TRANSLATE("Paper size:"), fPaper);

	// orientation
	fOrientation = new BPopUpMenu("orientation");
	fOrientation->SetRadioMode(true);

	BMenuField* orientation = new BMenuField("orientation", B_TRANSLATE("Orientation:"), fOrientation);

	count = fPrinterCap->CountCap(PrinterCap::kOrientation);
	if (count == 0) {
		AddOrientationItem(B_TRANSLATE("Portrait"), JobData::kPortrait);
		AddOrientationItem(B_TRANSLATE("Landscape"), JobData::kLandscape);
	} else {
	...
	if (!marked)
		item->SetMarked(true);
	BMenuField* resolution = new BMenuField("resolution", B_TRANSLATE("Resolution:"), fResolution);

	// scale
	BString scale;
	scale << (int)fJobData->GetScaling();
	fScaling = new BTextControl("scale", B_TRANSLATE("Scale [%]:"),
									scale.String(),
	                                NULL);
	int num;
	...
	// scale to fit
	fScaleToFit = NULL;
	if (fPrinterCap->Supports(PrinterCap::kScaleToFit)) {
		fScaleToFit = new BCheckBox("scaleToFit", B_TRANSLATE("Scale to fit page"),
			new BMessage(kMsgScaleToFitChanged));
		fScaleToFit->SetValue(fJobData->GetScaleToFit()
			? B_CONTROL_ON : B_CONTROL_OFF);
		fScaleToFit->SetTarget(this);
		// fit overrides the percentage: while it is ticked the field is
		// blank and disabled, the typed value is kept for when it is
		// unticked
		fScalingText = fScaling->Text();
		if (fJobData->GetScaleToFit()) {
			fScaling->SetText("");
			fScaling->SetEnabled(false);
		}
	}
	...
	if (fScaleToFit != NULL)
		settingsLayout->AddView(fScaleToFit, 1, 4);
...
PageSetupView::UpdateJobData()
{
	...
	bool scaleToFit = fScaleToFit != NULL
		&& fScaleToFit->Value() == B_CONTROL_ON;
	// while fit is ticked the field is blank and the percentage is not used
	float scaling = scaleToFit ? 100.0 : atoi(fScaling->Text());
	...
	fJobData->SetScaleToFit(scaleToFit);
...
{
	switch (msg->what) {
		case kMsgScaleToFitChanged:
			if (fScaleToFit->Value() == B_CONTROL_ON) {
				fScalingText = fScaling->Text();
				fScaling->SetText("");
				fScaling->SetEnabled(false);
			} else {
				fScaling->SetText(fScalingText.String());
				fScaling->SetEnabled(true);
			}
			break;
...
PageSetupDlg::PageSetupDlg(JobData *job_data, PrinterData *printer_data, const PrinterCap *printer_cap)
	: DialogWindow(BRect(100, 100, 160, 160),
		B_TRANSLATE("Paper setup"), B_TITLED_WINDOW_LOOK, B_MODAL_APP_WINDOW_FEEL,
