...
#define B_TRANSLATION_CONTEXT "JobSetupDlg"
...
// (not translated: static initializers run before the catalog is available)
...
JobSetupView::AttachedToWindow()
{
	...
	qualityBox->SetLabel(B_TRANSLATE("Quality"));
	...
	FillCapabilityMenu(fColorType, kMsgQuality, PrinterCap::kColor,
		fJobData->GetColor());
	BMenuField* colorMenuField = new BMenuField("color", B_TRANSLATE("Color:"), fColorType);
	...
	pageRangeBox->SetLabel(B_TRANSLATE("Page range"));

	fAll = new BRadioButton("all", B_TRANSLATE("Print all Pages"), new BMessage(kMsgRangeAll));

	BRadioButton *range = new BRadioButton("selection", B_TRANSLATE("Print selected Pages:"),
		new BMessage(kMsgRangeSelection));

	fFromPage = new BTextControl("from", B_TRANSLATE("From:"), "", NULL);
	fFromPage->SetAlignment(B_ALIGN_LEFT, B_ALIGN_RIGHT);
	AllowOnlyDigits(fFromPage->TextView(), 6);

	fToPage = new BTextControl("to", B_TRANSLATE("To:"), "", NULL);
	...
	if (first_page <= 1 && (last_page <= 0 || last_page == INT32_MAX)) {
	...
	BMenuField* paperSourceMenufield = new BMenuField("paperSource",
		B_TRANSLATE("Paper source:"), fPaperFeed);
	...
	BMenuField* pagesPerSheet = new BMenuField("pagesPerSheet",
		B_TRANSLATE("Pages per sheet:"), fNup);

	// duplex
	if (fPrinterCap->Supports(PrinterCap::kPrintStyle)) {
		fDuplex = new BCheckBox("duplex", B_TRANSLATE("Duplex"),
	...
	// copies
	fCopies = new BTextControl("copies", B_TRANSLATE("Number of copies:"), "", NULL);
	...
	// collate
	fCollate = new BCheckBox("collate", B_TRANSLATE("Collate"),
	...
	// reverse
	fReverse = new BCheckBox("reverse", B_TRANSLATE("Reverse order"),
	...
	pageSelectionBox->SetLabel(B_TRANSLATE("Page selection"));

	fAllPages = CreatePageSelectionItem("allPages", B_TRANSLATE("All pages"),
		JobData::kAllPages);
	fOddNumberedPages = CreatePageSelectionItem("oddPages",
		B_TRANSLATE("Odd-numbered pages"), JobData::kOddNumberedPages);
	fEvenNumberedPages = CreatePageSelectionItem("evenPages",
		B_TRANSLATE("Even-numbered pages"), JobData::kEvenNumberedPages);

	fPreview = new BCheckBox("preview", B_TRANSLATE("Show preview before printing"), NULL);
...
JobSetupView::CreateHalftoneConfigurationUI()
{
	...
	fDitherMenuField = new BMenuField("dithering", B_TRANSLATE("Dot Pattern:"),
		fDitherType);
	...
	// gamma
	fGamma = new JSDSlider("gamma", B_TRANSLATE("Gamma"), new BMessage(kMsgQuality),
		-300, 300);

	fGamma->SetLimitLabels(B_TRANSLATE("Lighter"), B_TRANSLATE("Darker"));
	...
	// ink density
	fInkDensity = new JSDSlider("inkDensity", B_TRANSLATE("Ink usage"),
		new BMessage(kMsgQuality), 0, 127);

	fInkDensity->SetLimitLabels(B_TRANSLATE("Min"), B_TRANSLATE("Max"));
...
JobSetupView::UpdateJobData()
{
	...
	if (B_CONTROL_ON == fAll->Value()) {
		// INT32_MAX is what applications expect for "all pages"
		// (BPrintJob::LastPage(), see also the Preview driver).
		first_page = 1;
		last_page  = INT32_MAX;
	} else {
...
JobSetupDlg::JobSetupDlg(JobData* jobData, PrinterData* printerData,
	const PrinterCap* printerCap)
	:
	DialogWindow(BRect(100, 100, 200, 200), B_TRANSLATE("Print job setup"),
