...
#define B_TRANSLATION_CONTEXT "GraphicsDriver"
...
GraphicsDriver::_SetupData(BFile* spoolFile)
{
	...
	if (fOrgJobData->GetCollate() && fPageCount > 1
		&& !(fPrinterCap->Supports(PrinterCap::kCopyCommand)
			&& fPrinterCap->Supports(PrinterCap::kCollateCommand))) {
		// collated copies: repeat the whole document, unless the printer
		// does that itself
...
// Renders the pages small into the band bitmap, without clipping, and
// returns the bounds of what was drawn in the application's coordinate
// system (origin = top left of the printable area, 1/72 inch units).
bool
GraphicsDriver::_MeasureContent(PageDataList* pages, BRect& content)
{
	BRect page = fOrgJobData->GetScaledPhysicalRect();
	if (page.Width() <= 0 || page.Height() <= 0)
		return false;

	// Content may extend up to one page beyond every edge.
	const float kRange = 3.0f;
	float bandWidth = fBitmap->Bounds().Width() + 1;
	float bandHeight = fBitmap->Bounds().Height() + 1;
	float scale = min(bandWidth / (kRange * page.Width()),
		bandHeight / (kRange * page.Height()));
	if (scale <= 0)
		return false;
	BPoint origin(page.Width(), page.Height());
	...
	for (PageDataList::iterator it = pages->begin(); it != pages->end();
		...
		do {
			...
			if (picture_data == NULL)
				break;
			fView->DrawPicture(picture_data->picture,
				origin + picture_data->point);
			fView->Sync();
			delete picture_data;
		} while (more);
	}
	...
	// get_valid_rect() takes the area to scan as input
	RECT rc;
	rc.left = 0;
	rc.top = 0;
	rc.right = (int)bandWidth - 1;
	rc.bottom = (int)bandHeight - 1;
	if (!get_valid_rect(fBitmap, &rc))
		return false;

	content.Set(rc.left / scale - origin.x, rc.top / scale - origin.y,
		(rc.right + 1) / scale - origin.x, (rc.bottom + 1) / scale - origin.y);
...
GraphicsDriver::_PrintPage(PageDataList* pages)
{
	...
	// Scale to fit: shrink and move the content so that it lies within
	// the printable area. In the application's coordinate system the
	// printable area is the rectangle (0, 0, width, height).
	bool fitting = false;
	float fit = 1.0f;
	BRect content;
	if (ScaleToFit() && fOrgJobData->GetNup() == 1
		&& _MeasureContent(pages, content)) {
		BRect printable = fOrgJobData->GetScaledPrintableRect();
		printable.OffsetTo(0, 0);
		if (!printable.Contains(content)) {
			fitting = true;
			fit = min(1.0f, min(printable.Width() / content.Width(),
				printable.Height() / content.Height()));
		}
	}
	...
	do {
		...
		for (PageDataList::iterator it = pages->begin(); it != pages->end();
			...
			if (fitting) {
				// left_top is the margin; the content's top left corner
				// has to end up there
				left_top.x = left_top.x / fit - content.left;
				left_top.y = left_top.y / fit - content.top;
				clip.Set(0, 0, clip.Width() / fit, clip.Height() / fit);
				clip.OffsetTo(left_top + content.LeftTop());
			} else
...
GraphicsDriver::_PrintJob(BFile* spoolFile)
{
	...
	if (success == false) {
		BAlert *alert;
		if (fTransport->CheckAbort())
			alert = new BAlert("", fTransport->LastError().c_str(), B_TRANSLATE("OK"));
		else
			alert = new BAlert("", B_TRANSLATE("Printer not responding."), B_TRANSLATE("OK"));
...
bool
GraphicsDriver::ScaleToFit() const
{
	return fOrgJobData->GetScaleToFit();
}
