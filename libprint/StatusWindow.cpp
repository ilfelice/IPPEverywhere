			...
			| B_AUTO_UPDATE_SIZE_LIMITS)
{
	...
	// the status bar
	fStatusBar = new BStatusBar("statusBar", B_TRANSLATE("Page: "));

	// the cancel button
	fHideButton = new BButton("hideButton",	B_TRANSLATE("Hide status"),
...
{
	Lock();
		fStatusBar->Reset(B_TRANSLATE("Page:  "));
...
StatusWindow::UpdateStatusBar(uint32 page, uint32 copy)
{
	Lock();
		...
		BString string3 = BString(B_TRANSLATE("Remaining document copies:  "));
		if (fDocumentCopy == true) {
			sprintf(buffer, "%d", (int)(fDocCopies));
			BString string4(buffer);
			string3.Append(string4);
		} else {
			string3 = BString(B_TRANSLATE("Remaining page copies:  "));
			char buffer[20];
...
StatusWindow::MessageReceived(BMessage *message)
{
	...
	switch(message->what) {
		case CANCEL_MSG:	// 'CancelButton' is pressed...
			fCancelled = true;
			fCancelButton->SetEnabled(false);
			fCancelButton->SetLabel(B_TRANSLATE("Job cancelled"));
			InvalidateLayout(true);
			break;
